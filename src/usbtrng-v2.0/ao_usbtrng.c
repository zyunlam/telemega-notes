...
ao_trng_fetch(void)
{
	...
	static uint16_t	adc_in[AO_USB_IN_SIZE];	/* twice as many as we need */
	uint32_t	kbytes = 1;
	uint32_t	count;
	int		usb_buf_id;
	int		i, j;
	...
	while (count--) {
		ao_adc_read(adc_in, AO_USB_IN_SIZE);
		rnd = (uint8_t *) adc_in;
		buf = buffer[usb_buf_id];
		for (i = 0; i < 32; i++) {
			for (j = 0; j < 4; j++)
				crc16 = (crc16 >> 8) ^ crc16_table[(crc16 ^ *rnd++) & 0xff];
			*buf++ = crc16;
		}
...
static const struct ao_cmds usbtrng_cmds[] = {
	{ ao_trng_fetch,	"f\0Fetch a block of numbers" },
	{ 0, NULL },
};
